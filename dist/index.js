...
var shouldSubscribeByName = (name, signalName, exact) =>
  exact && signalName
    ...
    : !name ||
      !signalName ||
      name === signalName ||
      convertToArrayPayload(name).some((currentName) => currentName && (currentName.startsWith(signalName) || signalName.startsWith(currentName)));
...
function deepEqual(object1, object2) {
  ...
  for (const key of keys1) {
    ...
    if (key !== "ref") {
      ...
      if (
        (isDateObject(val1) && isDateObject(val2)) || (isObject(val1) && isObject(val2)) || (Array.isArray(val1) && Array.isArray(val2))
          ? !deepEqual(val1, val2)
          : val1 !== val2
      ) {
        return false;
...
var getRuleValue = (rule) =>
  isUndefined(rule) ? rule : isRegex(rule) ? rule.source : isObject(rule) ? (isRegex(rule.value) ? rule.value.source : rule.value) : rule;

var hasValidation = (options) =>
  options.mount && (options.required || options.min || options.max || options.maxLength || options.minLength || options.pattern || options.validate);
...
function createFormControl(props = {}, flushRootRender) {
  ...
  const _getFieldArray = (name) =>
    compact(get(_stateFlags.mount ? _formValues : _defaultValues, name, props.shouldUnregister ? get(_defaultValues, name, []) : []));
  ...
  const register = (name, options = {}) => {
    ...
    field
      ? disabledIsDefined && set(_formValues, name, options.disabled ? undefined : get(_formValues, name, getFieldValue(field._f)))
...
function createFormContext(initialState) {
  ...
  const result = {
    Provider: ({ children, defaultValues, ...formProps }) => {
      ...
      return React.createElement(Context.Provider, { value: methods }, React.createElement(FormProvider, { ...methods }, children));
    },
    context: Context,
    ...
    useFormState: function (options) {
      const { control } = reactExports.useContext(Context);
    ...
    withFormProvider: function (Component, formProps) {
      return (props) => {
        ...
        return React.createElement(Provider, { ...formProps }, React.createElement(Component, { ...props }));
      };
    },
  };
  return result;
